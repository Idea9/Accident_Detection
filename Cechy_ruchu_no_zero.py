...
def get_motion_feature(file, startFrame, endFrame, returnOption):
    ...
    for frameId, frame in enumerate(frames):
        ...
        summary = sum(vLength) + 0.000001
        if summary > 10:
            totalMotion = np.append(totalMotion, summary)
        else:
            badFrames.append(frameId)

    print "ilosc zlych ramek", len(badFrames)
    i = 0
    angleMotion = []
